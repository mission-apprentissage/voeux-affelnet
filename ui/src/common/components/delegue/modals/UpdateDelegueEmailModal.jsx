...
export const UpdateDelegueEmailModal = ({ delegue, callback, isOpen, onClose }) => {
  const toast = useToast();

  const updateEmail = useCallback(
    async ({ form }) => {
      try {
        await _put(`/api/delegue/setEmail`, { email: form.email });
        toast({
          title: "Votre adresse courriel a été modifiée",
          description:
            "C'est à cette adresse que vous recevrez les mises à jour des listes de candidats pour les établissements pour lesquels une délégation de droit au téléchargement vous a été accordée.",
          status: "success",
          duration: 9000,
          isClosable: true,
        });
