...
describe("gestionnaireRoutes", () => {
  xit("Vérifie qu'un gestionnaire peut accéder à la liste des fichiers en étant authentifié", async () => {
    ...
    const response = await httpClient.get("/api/gestionnaire/fichiers", {
      headers: {
        ...auth,
      },
    });
  ...
  xit("Vérifie qu'on peut accéder à un fichier", async () => {
    ...
    const response = await httpClient.get("/api/gestionnaire/fichiers/0751234J.csv", {
      headers: {
        ...auth,
      },
    });
...
`
  ...
  xit("Vérifie qu'un événement est enregistré quand un gestionnaire télécharge un fichier", async () => {
    const { httpClient, createAndLogUser } = await startServer();
    const { auth } = await createAndLogUser("11111111100006", "password", {
      model: Gestionnaire,
      etablissements: [{ uai: "0751234J", voeux_date: DateTime.fromISO("2021-06-02T14:00:00.000Z").toJSDate() }],
    ...
    const found = await Gestionnaire.findOne({ username: "11111111100006" }).lean();
    assert.ok(found.voeux_telechargements[0].date);
  });

  xit("Vérifie qu'on indique au gestionnaire quand il y a une nouvelle version du fichier disponible", async () => {
    const { httpClient, createAndLogUser } = await startServer();
    const { auth } = await createAndLogUser("11111111100006", "password", {
      model: Gestionnaire,
      etablissements: [{ uai: "0751234J", voeux_date: DateTime.fromISO("2021-06-02T14:00:00.000Z").toJSDate() }],
  ...
  xit("Vérifie qu'on indique au gestionnaire si le fichier a déjà été téléchargé", async () => {
    const { httpClient, createAndLogUser } = await startServer();
    const { auth } = await createAndLogUser("11111111100006", "password", {
      model: Gestionnaire,
      etablissements: [{ uai: "0751234J", voeux_date: DateTime.fromISO("2021-06-02T14:00:00.000Z").toJSDate() }],
  ...
  xit("Doit rejeter un utilisateur qui n'est pas un gestionnaire", async () => {
    const { httpClient, createAndLogUser } = await startServer();
    const { auth } = await createAndLogUser("user1", "password");

    const response = await httpClient.get("/api/gestionnaire/fichiers/unknown.csv", {
      headers: {
        ...auth,
      },
    });
  ...
  xit("Doit rejeter un fichier UAI qui n'appartient pas au gestionnaire", async () => {
    const { httpClient, createAndLogUser } = await startServer();
    const { auth } = await createAndLogUser("11111111100006", "password", { model: Gestionnaire });

    const response = await httpClient.get("/api/gestionnaire/fichiers/0757890U.csv", {
      headers: {
        ...auth,
      },
    });
  ...
  xit("Doit rejeter un nom de fichier invalide", async () => {
    ...
    const response = await httpClient.get("/api/gestionnaire/fichiers/invalide.csv", {
      headers: {
        ...auth,
      },
    });
