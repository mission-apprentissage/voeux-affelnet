...
const importGestionnaires = require("../../src/jobs/importGestionnaires");
...
describe("importGestionnaires", () => {
  xit("Vérifie qu'on peut importer un cfa", async () => {
    ...
    const stats = await importGestionnaires(cfaCsv, {
      referentielApi: fakeReferentielApi,
    });

    const found = await Gestionnaire.findOne({ siret: "11111111100006" }, { _id: 0 }).lean();
  ...
  xit("Vérifie que le cfa a des voeux", async () => {
    ...
    await importGestionnaires(cfaCsv, {
      referentielApi: fakeReferentielApi,
    });
  ...
  xit("Vérifie qu'on peut ignorer les cfas invalides (pas d'emails)", async () => {
    ...
    const count = await Gestionnaire.countDocuments({ siret: "11111111100006" });
  ...
  xit("Vérifie qu'on peut mettre à jour uniquement certaines des informations", async () => {
    ...
    await importGestionnaires(createStream(`siret;email;etablissements\n11111111100006;contact@lycee.fr;0751234J`), {
      referentielApi: fakeReferentielApi,
    });
    ...
    const stats = await importGestionnaires(
  ...
  xit("Vérifie qu'on gère les erreurs durant l'import", async () => {
    ...
    const stats = await importGestionnaires(cfaCsv, options);

    const count = await Gestionnaire.countDocuments();
    assert.strictEqual(count, 0);
  ...
  xit("Vérifie qu'on gère les organismes inconnus dans le référentiel", async () => {
    ...
    const found = await Gestionnaire.findOne();
