...
const catalogue = async () => {
  ...
  // const findFormationCache = new Map();
  ...
  return {
    ...
    //     logger.error(
    //       e,
    //       `catalogue > findResponsableSiretAndEmail : Une erreur est survenue lors de l'appel au catalogue pour les valeurs ${uai_formateur} /
    //     ${cle_ministere_educatif} /
    //     ${siret_responsable}`
    //     );
    //     return null;
    ...
    findFormation: async (params) => {
      ...
      // if (findFormationCache.has(key)) {
      //   return findFormationCache.get(key);
      // }

      try {
        const formation = await catalogueApi.getFormation(params);

        // findFormationCache.set(JSON.stringify(key), formation);
        return formation;
      } catch (e) {
        // logger.error(
        //   e,
        //   `catalogue > findFormation : Une erreur est survenue lors de l'appel au catalogue pour les valeurs ${JSON.stringify(
        //     params
        //   )}`
        // );
        return null;
